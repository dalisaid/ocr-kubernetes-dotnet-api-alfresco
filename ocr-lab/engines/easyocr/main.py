...
# ── EasyOCR reader cache — preserved from original ────────────
# Loading a language model takes 10-30s, cache it per language key
_reader_cache = {}
...
# ── FastAPI app ────────────────────────────────────────────────
app = FastAPI(title="EasyOCR Engine", version="2.0.0")
...
async def startup():
    try:
        get_reader("en")
        logger.info("EasyOCR English reader ready")
            ...
            try:
                with OCR_DURATION.labels(engine=ENGINE_NAME).time():
                    with tracer.start_as_current_span("ocr-processing"):
                        # EasyOCR is synchronous and CPU-heavy —
                        ...
                        results = await loop.run_in_executor(
                            None, lambda: reader.readtext(image_np, detail=0)
                        )
                        text = "\n".join(results)
